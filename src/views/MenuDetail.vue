                ...
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="12" md="12">
                      <v-select
                        v-validate="'required'"
                        :items="mDataArrayGroupMenu"
                        label="กลุ่มเมนู"
                        item-text="cgm_name"
                        item-value="id"
                        v-model="editedItem.group_menu_id"
                        outlined
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Route", value: "cmd_route" },
      { text: "Group Menu", value: "cgm_name" },
  ...
  methods: {
    async changeSPGroupMenu(selectObj) {
      // console.log(selectObj)
      this.editedItem.group_menu_id = selectObj;
      this.editedItem2.group_menu_id = selectObj;
      //   alert(selectObj);
    },
    async loadSPGroupMenu() {
      let result = await api.getGroupMenu();
      this.mDataArrayGroupMenu = result.data;
    ...
    async initialize() {
      ...
      //   alert("initialize");
      const res_get = await api.getAllDetailGroupMenu();
    ...
    async deleteItemConfirm() {
      ...
      const res_del = await api.deleteMenuDetail(this.edit_item_id);
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.editedItem2.cmd_name = this.editedItem.cmd_name;
        this.editedItem2.cmd_route = this.editedItem.cmd_route;
        this.editedItem2.group_menu_id = this.editedItem.group_menu_id;

        const res_edit = await api.updateMenuDetail(
          this.edit_item_id,
          this.editedItem2
        );
        ...
        const res_add = await api.createMenuDetail(this.editedItem);
