                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        v-model="editedItem.harvest_truck_license_plate"
                        label="ชื่อรถบรรทุก"
                        outlined
                      ></v-text-field>
                    </v-col>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "ชื่อ", value: "harvest_truck_license_plate" },
  ...
  methods: {
    async initialize() {
      ...
      //   alert("initialize");
      const res_get = await api.getHarvestTruck();
    ...
    async deleteItemConfirm() {
      ...
      const res_del = await api.deleteHarvestTruck(this.edit_item_id);
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.editedItem2.harvest_truck_license_plate = this.editedItem.harvest_truck_license_plate;
        const res_edit = await api.updateHarvestTruck(
          this.edit_item_id,
          this.editedItem2
        );
        ...
        const res_add = await api.addHarvestTruck(this.editedItem);
