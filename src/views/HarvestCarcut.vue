                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        v-model="editedItem.harvest_carcut_name"
                        label="ชื่อรถตัดอ้อย"
                        outlined
                      ></v-text-field>
                    </v-col>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "ชื่อ", value: "harvest_carcut_name" },
  ...
  methods: {
    async initialize() {
      ...
      //   alert("initialize");
      const res_get = await api.getHarvestCarcut();
    ...
    async deleteItemConfirm() {
      ...
      const res_del = await api.deleteHarvestCarcut(this.edit_item_id);
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.editedItem2.harvest_carcut_name = this.editedItem.harvest_carcut_name;
        const res_edit = await api.updateHarvestCarcut(
          this.edit_item_id,
          this.editedItem2
        );
        ...
        const res_add = await api.addHarvestCarcut(this.editedItem);
