                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        v-model="editedItem.cane_plow_type_name"
                        label="ชื่อสถานะเตรียมแปลง"
                        outlined
                      ></v-text-field>
                    </v-col>
                  </v-row>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "ชื่อ", value: "cane_plow_type_name" },
  ...
  methods: {
    async initialize() {
      ...
      //   alert("initialize");
      const res_get = await api.getCanePlowType();
    ...
    async deleteItemConfirm() {
      ...
      const res_del = await api.deleteCanePlowType(this.edit_item_id);
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        this.editedItem2.cane_plow_type_name = this.editedItem.cane_plow_type_name;
        const res_edit = await api.updateCanePlowType(
          this.edit_item_id,
          this.editedItem2
        );
        ...
        const res_add = await api.addCanePlowType(this.editedItem);
